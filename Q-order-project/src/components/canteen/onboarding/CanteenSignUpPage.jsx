...
import './CanteenSignUpPage.css';

const CanteenSignUpPage = ({ onSignUpSuccess, onBackToLogin, onRoleChange }) => {
  ...
  const handleInputChange = (field, value) => {
    ...
    if (field === 'role' && value !== 'Canteen' && onRoleChange) {
      onRoleChange(value);
    }
  };
  ...
  return (
    <div className="canteen-signup-page-wrapper">
      <div className="canteen-signup-card-container">
        {onBackToLogin && (
          <button className="canteen-signup-back-btn" onClick={onBackToLogin} aria-label="Go back">
            <ChevronLeft size={24} />
          </button>
        )}

        <h1 className="canteen-signup-title">Sign Up</h1>
        <p className="canteen-signup-subtitle">Please sign up to get started</p>

        <form onSubmit={handleSubmit} className="canteen-signup-form">
          ...
          <div className="input-field-group">
            <select
              ...
              className={`pill-input pill-select ${!formData.university ? 'placeholder-active' : ''}`}
            >
